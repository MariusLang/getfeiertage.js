...
test('get Weiberfastnacht', () => {
  expect(Feiertage.getWeiberfastnacht(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-02-16`).valueOf());
  expect(Feiertage.getWeiberfastnacht(Feiertage.formatDE, year)).toBe(`16.02.${year}`);
});
...
test('get Rosenmontag', () => {
  expect(Feiertage.getRosenmontag(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-02-20`).valueOf());
  expect(Feiertage.getRosenmontag(Feiertage.formatDE, year)).toBe(`20.02.${year}`);
});

test('get Fastnachtsdienstag', () => {
  expect(Feiertage.getFastnachtsdienstag(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-02-21`).valueOf());
  expect(Feiertage.getFastnachtsdienstag(Feiertage.formatDE, year)).toBe(`21.02.${year}`);
});

test('get Aschermittwoch', () => {
  expect(Feiertage.getAschermittwoch(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-02-22`).valueOf());
  expect(Feiertage.getAschermittwoch(Feiertage.formatDE, year)).toBe(`22.02.${year}`);
});
...
test('get Gründonnerstag', () => {
  expect(Feiertage.getGruendonnerstag(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-04-06`).valueOf());
  expect(Feiertage.getGruendonnerstag(Feiertage.formatDE, year)).toBe(`06.04.${year}`);
});

test('get Karfreitag', () => {
  expect(Feiertage.getKarfreitag(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-04-07`).valueOf());
  expect(Feiertage.getKarfreitag(Feiertage.formatDE, year)).toBe(`07.04.${year}`);
});

test('get Ostersonntag', () => {
  expect(Feiertage.getOstersonntag(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-04-09`).valueOf());
  expect(Feiertage.getOstersonntag(Feiertage.formatDE, year)).toBe(`09.04.${year}`);
});

test('get Ostermontag', () => {
  expect(Feiertage.getOstermontag(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-04-10`).valueOf());
  expect(Feiertage.getOstermontag(Feiertage.formatDE, year)).toBe(`10.04.${year}`);
});
...
test('get Christi Himmelfahrt', () => {
  expect(Feiertage.getChristiHimmelfahrt(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-05-18`).valueOf());
  expect(Feiertage.getChristiHimmelfahrt(Feiertage.formatDE, year)).toBe(`18.05.${year}`);
});

test('get Pfingstsonntag', () => {
  expect(Feiertage.getPfingstsonntag(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-05-28`).valueOf());
  expect(Feiertage.getPfingstsonntag(Feiertage.formatDE, year)).toBe(`28.05.${year}`);
});

test('get Pfingstmontag', () => {
  expect(Feiertage.getPfingstmontag(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-05-29`).valueOf());
  expect(Feiertage.getPfingstmontag(Feiertage.formatDE, year)).toBe(`29.05.${year}`);
});

test('get Fronleichnam', () => {
  expect(Feiertage.getFronleichnam(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-06-08`).valueOf());
  expect(Feiertage.getFronleichnam(Feiertage.formatDE, year)).toBe(`08.06.${year}`);
});
...
test('get Buß und Bettag', () => {
  expect(Feiertage.getBussUndBettag(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-11-22`).valueOf());
  expect(Feiertage.getBussUndBettag(Feiertage.formatDE, year)).toBe(`22.11.${year}`);
});
...
test('get 1. Advent', () => {
  expect(Feiertage.getAdvent1(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-12-03`).valueOf());
  expect(Feiertage.getAdvent1(Feiertage.formatDE, year)).toBe(`03.12.${year}`);
});

test('get 2. Advent', () => {
  expect(Feiertage.getAdvent2(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-12-10`).valueOf());
  expect(Feiertage.getAdvent2(Feiertage.formatDE, year)).toBe(`10.12.${year}`);
});

test('get 3. Advent', () => {
  expect(Feiertage.getAdvent3(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-12-17`).valueOf());
  expect(Feiertage.getAdvent3(Feiertage.formatDE, year)).toBe(`17.12.${year}`);
});

test('get 4. Advent', () => {
  expect(Feiertage.getAdvent4(Feiertage.formatDateObj, year).valueOf()).toBe(new Date(`${year}-12-24`).valueOf());
  expect(Feiertage.getAdvent4(Feiertage.formatDE, year)).toBe(`24.12.${year}`);
});
